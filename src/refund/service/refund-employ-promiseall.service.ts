...
export class RefundEmployService {
  ...
  async processEmployData(
    ...
    try {
      ...
      //     } catch (error) {
      //       this.logger.error(
      //         `고용산재데이터수집 실패 관리번호: ${gwanriNo}, year: ${year}:`,
      //         JSON.stringify(error, null, 2),
      //       )
      //       throw AppException.from(
      //         error,
      //         `사업장 ${gwanriNo} ${year}년 데이터 수집 실패`,
      //         'employ-data',
      //         'refund-employ',
      //       )
      //     }
      ...
      for (const gwanriNo of gwanriNoList) {
        for (let year = startYear; year <= endYear; year++) {
          try {
            ...
            // employment 테이블이 생성된 후 salary 데이터 업데이트
  ...
  private async collectEmploymentData(
    ...
    console.log('collect employee', `${gwanriNo}<=>${year}`)
    ...
    try {
      ...
      if (employData?.Result?.dsOutList.length === 0) {
        console.log('고용현황 데이터가 없습니다', year.toString())
        return
      }
      ...
      await Promise.all([
        this.supabaseRefund.uploadExcelToStorage(
          ...
          userId,
          `${userId}/${gwanriNo}_${year}_employment.xlsx`,
        ),
      ...
      throw AppException.from(
        error,
        `고용정보 데이터 수집 실패 (사업장: ${gwanriNo}, 년도: ${year})`,
        'employ-data',
        'employ-info',
      )
    }
  ...
  private async processEmploymentData(
    ...
    try {
      ...
      const employees = employmentData.dsOutList.map(data => ({
        ...
        employeeRegNo: data.GEUNROJA_RGNO,
        qualificationGetDate: data.SJB_JAGYEOK_CHWIDEUK_DT,
        qualificationLostDate: data.SJB_JAGYEOK_SANGSIL_DT,
        employeeNo: data.GEUNROJA_WONBU_NO,
      }))
